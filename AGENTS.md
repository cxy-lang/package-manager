...
import { OAuthClient } from "@oauth"           // installed package (@ prefix)
import { MyThing } from "@mypkg/submodule.cxy" // installed package submodule
...
There are three kinds of import paths:

| Prefix | Meaning | Example |
|--------|---------|---------|
| `"stdlib/..."` | Standard library module | `import { Vector } from "stdlib/vector.cxy"` |
| `"./"` or `"../"` | Relative path to local file | `import { Foo } from "./foo.cxy"` |
| `"@name"` | Installed external package | `import { OAuthClient } from "@oauth"` |

```cxy
// ✅ Preferred — import from top-level package when types are exported there
import { OAuthClient, GitHub, getGitHubUser } from "@oauth"

// ❌ Wrong — bare name without @ is NOT a valid package import
import { OAuthClient } from "oauth"

// ✅ Acceptable — submodule path when type is NOT exported in index.cxy
import { SomeInternal } from "@oauth/src/internal.cxy"

// ❌ Avoid — do not use submodule paths for types already in index.cxy
import { GitHub } from "@oauth/providers/github.cxy"  // GitHub is in @oauth
```

Always check the package's `index.cxy` first to see what is exported. Prefer the
top-level `@pkgname` import whenever the type is available there. Only reach into
`@pkg/subpath.cxy` if the specific type is **not** re-exported by `index.cxy`.
...
```cxy
// __copy! is ONLY needed when the expression is a reference (&T)
func process(val: T) { }

var x: &T = returnsReference()
process(__copy!(x))  // ✅ explicit copy required — x is a &T reference

// Without __copy!:
process(x)           // ❌ may fail — cannot implicitly copy a reference

// NOT needed for class values (ref counting handles it automatically)
var s: String = getString()
process(s)           // ✅ fine — s is a String value, not a &String reference

// NOT needed when accessing fields through a reference
func useField(user: &User) {
    process(user.login)  // ✅ fine — user.login is a String value
}
...
// Key principle: __copy! is specifically for &T → T, not for all class types
```
...
// Notes:
// - 'this' is a reference (&Type) in classes
// - 'this' is a pointer (^Type) in structs
// - ^Type means pointer type, &Type means reference type
...
func handler(): void {
    crash(0) catch {
        ...
        println("Error message: ", ex!.what())  // .what() returns the error message
    }
}

// Get error message with .what()
func logError(): void {
    doSomething() catch {
        var message = ex!.what()  // ✅ always use .what() for error messages
        println("Error occurred: ", message)
    }
}

// Stack traces are automatically included when printing exceptions
// Use .what() to get the error message with stack trace
func handler(): void {
    crash(0) catch {
        println("Error: ", ex!.what())
        // Prints:
...
// Rules:
// - Use ex!.what() to get error message (includes stack trace)
// - Exceptions automatically include stack traces
// - Use 'yield' to return a default value from catch block
// - Use 'raise' without args to re-raise caught exception
